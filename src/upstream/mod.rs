...
const PLACEHOLDER_REVISION: &str = "main";
impl Upstream {
    ...
    pub async fn scan(&self) -> anyhow::Result<Vec<UpstreamMatch>> {
        ...
        for root in &self.roots {
            let mut matches = root.scan(self, PLACEHOLDER_REVISION).await?;
...
impl SourceRoot {
    pub async fn scan(
        ...
        // Build glob filters
        // DESIGN can globs be pre-validated? Need lifetime on SourceRoot.
        // let includes = wax::any(&self.includes.iter().map(String::as_str).collect())
        //     .with_context(|| format!("Compile includes for Source Root {}", &self.id))?;
        // let excludes = wax::any(&self.excludes.iter().map(String::as_str).collect())
        ...
        for entry in entries {
            ...
            if !self.lang.should_parse(&path.to_path_buf()) {
                continue;
            };
            trace!(
